...
export function AdminCrownButton({ onAdminLogin, onFarmOwnerLogin, onBackToAdmin }: AdminCrownButtonProps) {
  ...
  const handleFarmOwnerClick = () => {
    console.log('🎯 Farm Owner clicked');
    setShowMenu(false);
    if (onFarmOwnerLogin) {
      onFarmOwnerLogin();
    } else {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-28 left-4 sm:bottom-32 sm:left-6 z-[60] flex items-center gap-3">
        {/* Back to Admin Button - Only show when logged in */}
        {isLoggedIn && onBackToAdmin && (
          <button
            onClick={handleBackToAdmin}
            className="group touch-manipulation active:scale-90 transition-all duration-300"
          ...
          >
            <div className="relative">
              ...
              {/* Button */}
              <div
                className="relative px-3 py-2.5 sm:px-4 sm:py-3 rounded-full flex items-center gap-2 transform transition-all duration-300 group-hover:scale-105 group-hover:shadow-2xl"
                style={{
        ...
        <button
          ...
          className="group touch-manipulation active:scale-90 transition-all duration-300 w-14 h-14 sm:w-[60px] sm:h-[60px] rounded-full"
      ...
      {showMenu && (
        <div
          className="fixed bottom-28 left-4 sm:bottom-32 sm:left-6 z-[70] w-72 sm:w-64 max-w-[calc(100vw-2rem)] rounded-2xl overflow-hidden transition-all duration-300 transform origin-bottom-left mb-20"
          style={{
