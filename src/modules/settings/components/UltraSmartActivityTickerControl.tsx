...
import {
  ...
  TrendingUp, Users, Award, Zap, CheckCircle, Clock, Play, Pause,
  BarChart3, Activity, Filter, Search, Edit3, Copy, Download, Upload,
  AlertTriangle, Star, Calendar, MapPin, DollarSign, TreePine, Home,
  XCircle, ChevronDown, ChevronUp, Layers, Globe, Maximize2, Minimize2
} from 'lucide-react';
...
export function UltraSmartActivityTickerControl() {
  // State Management
  const [activeSection, setActiveSection] = useState<'control' | 'analytics' | 'preview'>('control');
  ...
  const calculateStats = () => {
    ...
    const visible = platformActivities.filter(a => a.is_visible).length;
    const hidden = platformActivities.filter(a => !a.is_visible).length;
    const todayCount = platformActivities.filter(a => new Date(a.timestamp) >= today).length;
    const weekCount = platformActivities.filter(a => new Date(a.timestamp) >= weekAgo).length;
  ...
  const bulkToggleVisibility = async (visible: boolean) => {
    ...
    try {
      ...
      if (error) throw error;
      showNotification('success', `✅ تم ${visible ? 'إظهار' : 'إخفاء'} ${selectedItems.length} نشاط`);
      setSelectedItems([]);
      loadPlatformActivities();
    } catch (error) {
  ...
  const filteredActivities = platformActivities
    .filter(activity => {
      const matchesSearch =
        activity.activity_title_ar.toLowerCase().includes(searchQuery.toLowerCase()) ||
        activity.activity_title_en.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    .sort((a, b) => {
      if (sortBy === 'date') {
        ...
        return a.activity_title_ar.localeCompare(b.activity_title_ar);
      }
    });
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-lg text-gray-600 dark:text-gray-400 font-medium">
                إدارة متطورة للشريط المتحرك مع معاينة مباشرة
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`
                flex items-center gap-2 px-4 py-3 rounded-2xl font-bold transition-all shadow-lg
                ${liveMode
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          {[
            ...
            { label: 'مرئية', value: stats.visible, icon: Eye, color: 'from-green-500 to-emerald-600', bg: 'bg-green-50 dark:bg-green-900/20' },
            { label: 'مخفية', value: stats.hidden, icon: EyeOff, color: 'from-gray-500 to-slate-600', bg: 'bg-gray-50 dark:bg-gray-900/20' },
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`
                      flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl font-bold transition-all
                      ${isActive
                  ...
                  >
                    ...
                    {tab.count !== undefined && (
                      <span className={`
                        px-2 py-0.5 text-xs rounded-full font-black
                ...
                <div className="flex flex-col gap-4 mb-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <select
                      ...
                      className="px-4 py-2 bg-gray-50 dark:bg-gray-900/50 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white font-medium"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <span>{new Date(activity.timestamp).toLocaleString('ar-SA', { dateStyle: 'short', timeStyle: 'short' })}</span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      الشريط المتحرك يتحدث تلقائياً عند أي تغيير في الأنشطة أو الإعدادات
                    </div>
                  </div>
                </div>
              </div>
