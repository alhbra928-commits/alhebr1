...
import {
  ...
  User, Phone, Archive, X, FileText, Zap, Filter,
  AlertCircle, CheckCircle, Mail, MailOpen, Reply, Timer
} from 'lucide-react';
...
type FilterType = 'all' | 'unanswered' | 'answered' | 'new';

export const EnhancedSmartInboxPage: React.FC = () => {
  const [threads, setThreads] = useState<InboxThread[]>([]);
  const [filteredThreads, setFilteredThreads] = useState<InboxThread[]>([]);
  ...
  useEffect(() => {
    applyFilter();
  }, [threads, activeFilter, searchTerm]);
  ...
  const applyFilter = () => {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(thread =>
        thread.user_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        thread.user_phone.includes(searchTerm) ||
    ...
    switch (activeFilter) {
      case 'unanswered':
        filtered = filtered.filter(thread =>
          thread.awaiting_staff_reply === true || !thread.has_staff_reply
        );
        break;
      case 'answered':
        filtered = filtered.filter(thread => thread.has_staff_reply === true);
        break;
      case 'new':
  ...
  const getThreadStatusColor = (thread: InboxThread) => {
    // المحادثات التي لم يتم الرد عليها - أحمر
    if (thread.awaiting_staff_reply || !thread.has_staff_reply) {
      return 'border-r-4 border-red-500 bg-red-500/5';
    }
    // المحادثات التي تم الرد عليها - أخضر
    if (thread.has_staff_reply) {
      return 'border-r-4 border-green-500 bg-green-500/5';
    }
    return '';
  };

  const getThreadStatusIcon = (thread: InboxThread) => {
    if (thread.awaiting_staff_reply || !thread.has_staff_reply) {
      return (
        <div className="flex items-center gap-1 text-red-400 bg-red-500/10 px-2 py-1 rounded">
          <AlertCircle className="w-3 h-3" />
          <span className="text-xs">يحتاج رد</span>
        </div>
      );
    }
    if (thread.has_staff_reply) {
  ...
  const getFilteredCount = () => {
    const counts = {
      all: threads.length,
      unanswered: threads.filter(t => t.awaiting_staff_reply || !t.has_staff_reply).length,
      answered: threads.filter(t => t.has_staff_reply).length,
      new: threads.filter(t => t.unread_count > 0).length
    };
    return counts;
  };
  ...
  return (
    <div className="h-screen flex bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900" dir="rtl">
      ...
      <div className="absolute top-0 left-0 right-0 bg-gray-800/90 backdrop-blur-sm border-b border-gray-700 z-10">
        {/* الإحصائيات العامة */}
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between border-b border-gray-700/50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-2">
          ...
          <span className="text-gray-400 text-sm ml-2">تصنيف:</span>
      ...
      <div className="w-96 bg-gray-800/50 border-l border-gray-700 flex flex-col" style={{ marginTop: '140px' }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredThreads.map((thread) => (
            <button
              ...
              } ${getThreadStatusColor(thread)}`}
            >
              <div className="flex items-start gap-3">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${
                  thread.awaiting_staff_reply || !thread.has_staff_reply
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      {thread.staff_reply_count && thread.staff_reply_count > 0 && (
                        <span className="text-xs text-green-400 flex items-center gap-1">
                          <Reply className="w-3 h-3" />
                          {thread.staff_reply_count}
                        </span>
                      )}
                    </div>
                    {getThreadStatusIcon(thread)}
                  </div>
      ...
      <div className="flex-1 flex flex-col" style={{ marginTop: '140px' }}>
        {selectedThread ? (
          <>
            {/* رأس المحادثة */}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    selectedThread.awaiting_staff_reply || !selectedThread.has_staff_reply
                      ? 'bg-gradient-to-br from-red-500 to-red-600'
                      : 'bg-gradient-to-br from-green-500 to-emerald-600'
                  }`}>
                  ...
                  {getThreadStatusIcon(selectedThread)}
                </div>
            ...
            <form onSubmit={handleSendMessage} className="bg-gray-800/70 backdrop-blur-sm border-t border-gray-700 p-4">
              <div className="flex gap-2">
                <input
                  ...
                  disabled={sending}
                  className="flex-1 px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-green-500 focus:border-transparent disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={!messageText.trim() || sending}
                  className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 font-medium"
                >
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm mt-2">
                {counts.unanswered > 0 && `لديك ${counts.unanswered} محادثة تحتاج للرد`}
              </p>
            </div>
          </div>
        )}
      ...
      <div className="w-80 bg-gray-800/50 border-r border-gray-700 flex flex-col" style={{ marginTop: '140px' }}>
        <div className="p-4 border-b border-gray-700">
          <h3 className="text-white font-semibold flex items-center gap-2">
            <FileText className="w-5 h-5 text-blue-400" />
            القوالب الجاهزة
          </h3>
          <p className="text-xs text-gray-400 mt-1">اضغط لإرسال قالب سريع</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {templates.map((template) => (
            <button
              ...
              className="w-full text-right p-3 bg-gray-700/50 hover:bg-gray-700 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed group border border-transparent hover:border-green-500/30"
            >
