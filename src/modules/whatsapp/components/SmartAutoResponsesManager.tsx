            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {response.use_hybrid_message && (
                    <span
                      className="px-2 py-1 bg-gradient-to-r from-purple-600/30 to-pink-600/30 text-purple-300 text-xs rounded flex items-center gap-1 cursor-help"
                      title="يستخدم الرسالة الهجينة (يدوية + تلقائية): نوع العميل، المصدر، التاريخ"
                    >
                      ✉️ هجين
                    </span>
                  )}
              ...
              <button
                onClick={() => {
                  ...
                  setFormData({
                    ...
                    use_hybrid_message: response.use_hybrid_message || false
                  });
                  setShowAddModal(true);
                }}
          ...
          <div className="bg-gray-800 rounded-2xl p-6 max-w-2xl w-full border border-gray-700">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                ...
                {/* خيار استخدام الرسالة الهجينة */}
                {formData.whatsapp_business_link && formData.whatsapp_business_link.trim() !== '' && (
                  <div className="mt-4 p-3 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-lg">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input
                        type="checkbox"
                        checked={formData.use_hybrid_message || false}
                        onChange={(e) => setFormData({ ...formData, use_hybrid_message: e.target.checked })}
                        className="w-5 h-5 bg-gray-700 border-2 border-purple-500/50 rounded cursor-pointer checked:bg-purple-600 checked:border-purple-600 focus:ring-2 focus:ring-purple-500/50"
                      />
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-white font-medium">✉️ استخدام الرسالة المرافقة الهجينة</span>
                          <span className="px-2 py-0.5 bg-purple-600/30 text-purple-300 text-xs rounded-full">ذكي</span>
                        </div>
                        <p className="text-gray-400 text-xs mt-1">
                          عند تفعيل هذا الخيار، سيتم إضافة الرسالة الأساسية من الإعدادات + بيانات المستخدم التلقائية (نوع العميل، المصدر، التاريخ) إلى رابط واتساب الأعمال
                        </p>
                      </div>
                    </label>

                    {formData.use_hybrid_message && (
                      <div className="mt-3 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                        <div className="flex items-start gap-2">
                          <div className="text-green-400 mt-0.5">✓</div>
                          <div className="text-xs text-green-300">
                            <strong>الرسالة الهجينة مفعّلة!</strong>
                            <p className="text-gray-300 mt-1">
                              عند فتح الرابط، سيرى الموظف الرسالة اليدوية + معلومات تلقائية:
                              <br />• نوع العميل (مستثمر/صاحب مزرعة/زائر)
                              <br />• المصدر (الصفحة التي جاء منها)
                              <br />• التاريخ والوقت بالتنسيق العربي
                            </p>
                          </div>
                        </div>
                      </div>
