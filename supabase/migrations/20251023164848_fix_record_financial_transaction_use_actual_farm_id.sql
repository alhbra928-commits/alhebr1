/*
  # Fix farm_revenue_snapshots to Use Actual farm_id

  1. Problem
    - Function inserts smart_farm_finances.id into farm_revenue_snapshots.farm_id
    - But farm_revenue_snapshots.farm_id is a FK to farms table
    - Need to get the actual farm_id from farms table
    
  2. Solution
    - Query farms table to get the real farm_id
    - Use that in the snapshot INSERT
*/
...
BEGIN
  ...
  -- Get the actual farm_id from farms table
  SELECT id INTO v_actual_farm_id
  FROM farms
  WHERE farm_code = p_farm_code;

  IF v_actual_farm_id IS NULL THEN
    RAISE EXCEPTION 'Farm not found for code: %', p_farm_code;
  END IF;
  ...
  IF p_transaction_type = 'investor_revenue' THEN
    ...
    -- Create revenue snapshot with ACTUAL farm_id from farms table
    ...
    SELECT 
      v_actual_farm_id, sff.farm_code, sff.total_revenue_collected, 
      sff.total_investors, sff.total_trees_sold,
      sff.financial_completion_percentage, sff.coverage_percentage, 
      sff.marketing_amount, sff.actual_amount,
      sff.actual_amount - sff.total_revenue_collected, 'new_investment',
