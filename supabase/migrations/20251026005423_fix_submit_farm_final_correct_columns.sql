/*
  # الإصلاح النهائي - submit_farm_for_review
  
  المشكلة:
  - الدالة تستخدم أعمدة غير موجودة
  - reserved_trees ❌ (لا يوجد)
  - region_ar ❌ (استخدم region)
  - payment_grace_period_days ❌ (استخدم payment_grace_period)
  
  الحل:
  - استخدام الأعمدة الموجودة فعلاً في الجدول
*/
...
BEGIN
  ...
  -- إنشاء المزرعة (أعمدة موجودة فقط)
  INSERT INTO farms (
    owner_id,
    farm_code,
    name_ar,
  ...
  VALUES (
    p_profile_id,
    v_farm_code,
    'مزرعة ' || p_region,                    -- name_ar
    'Farm ' || p_region,                     -- name_en
    p_city || ', ' || p_region,              -- location
    p_region,
    p_city,
    p_deed_number,
    p_total_farm_area,
    p_farm_area_unit,
    p_total_farm_area,                       -- area_sqm
    p_farm_type,
    v_total_trees,
    v_total_trees,                           -- available_trees = total في البداية
    p_actual_total_price,
    p_price_per_tree,
    p_actual_total_price * 0.08,             -- expected_annual_return (8%)
