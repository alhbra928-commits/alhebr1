/*
  # إصلاح مرجع audit_log في دالة farm_owner_login_or_create

  1. المشكلة
    - الدالة تستخدم `audit_log` بدون `s`
    - الجدول الصحيح هو `audit_logs`

  2. الإصلاح
    - تحديث الدالة لتستخدم `audit_logs` الصحيح
*/

-- إعادة إنشاء الدالة بالمرجع الصحيح
