/*
  # Fix record_financial_transaction Function - Column Name Mismatch

  1. Problem
    - Function tries to insert into `farm_id` column
    - Actual table has `farm_finance_id` column
    - This causes trigger to fail silently

  2. Solution
    - Update INSERT statement to use correct column name `farm_finance_id`
    - Also update farm_revenue_snapshots INSERT to use `farm_finance_id`
*/
...
BEGIN
  ...
  -- Insert transaction record with correct column name
  ...
  IF p_transaction_type = 'investor_revenue' THEN
    ...
    -- Create revenue snapshot with correct column name
    INSERT INTO farm_revenue_snapshots (
      farm_finance_id, farm_code, total_revenue, total_investors, total_trees_sold,
