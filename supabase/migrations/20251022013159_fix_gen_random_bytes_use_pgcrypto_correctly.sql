/*
  # إصلاح استخدام gen_random_bytes - استخدام pgcrypto بشكل صحيح
  
  ## المشكلة
  - Error: function gen_random_bytes(integer) does not exist
  - الدالة تستخدم gen_random_bytes لكنها غير متاحة بشكل مباشر
  - pgcrypto مُفعل لكن يجب استخدامه بشكل صحيح
  
  ## الحل
  1. استخدام gen_random_uuid() لتوليد رمز التحقق (verification_token)
  2. أو استخدام encode(digest(...), 'hex') من pgcrypto
  3. تحديث الدالة لاستخدام الطريقة الصحيحة
  
  ## البديل الآمن
  - بدلاً من: encode(gen_random_bytes(16), 'hex')
  - نستخدم: encode(digest(gen_random_uuid()::text || now()::text, 'sha256'), 'hex')
  - أو نستخدم مباشرة: replace(gen_random_uuid()::text, '-', '')
*/

-- حذف الدالة القديمة
DROP FUNCTION IF EXISTS auto_migrate_to_documentation(uuid, uuid, text);

-- إنشاء الدالة مع الإصلاح
...
BEGIN
  ...
  -- ✅ استخدام gen_random_uuid بدلاً من gen_random_bytes
...
'إصدار شهادة تملك من حجز معتمد - يستخدم gen_random_uuid بدلاً من gen_random_bytes';
