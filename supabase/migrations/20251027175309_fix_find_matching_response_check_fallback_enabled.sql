/*
  # إصلاح دالة find_matching_auto_response للتحقق من fallback_enabled
  
  المشكلة:
  - الدالة تبحث عن is_default_fallback = true فقط
  - لا تتحقق من fallback_enabled = true
  
  الحل:
  - التحقق من أن الرد الافتراضي مفعّل (fallback_enabled = true)
  - فقط الرد الافتراضي المفعّل يُستخدم
  - يجب أن يعيد الرابط مع الرد
*/
...
BEGIN
  ...
  WHERE deleted_at IS NULL
    ...
    AND fallback_enabled = true  -- ✅ التحقق من التفعيل!
    AND response_ar IS NOT NULL 
    AND TRIM(response_ar) != ''
  ORDER BY
    priority DESC,
...
'دالة البحث عن رد تلقائي - ترجع الرد الافتراضي المفعّل فقط (fallback_enabled = true) مع الرابط';
