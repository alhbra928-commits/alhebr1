/*
  # إصلاح trigger التوثيق - إضافة verification_token
  
  1. التغييرات
    - توليد verification_token تلقائياً عند إنشاء التوثيق
    
  2. الأمان
    - كل توثيق يحصل على token فريد للتحقق
*/
...
BEGIN
  ...
  IF NEW.booking_status = 'documented' AND (OLD.booking_status IS DISTINCT FROM NEW.booking_status) THEN
    ...
    -- توليد token للتحقق
    v_verification_token := UPPER(SUBSTRING(MD5(NEW.id::TEXT || NOW()::TEXT || 'verify') FROM 1 FOR 16));
...
COMMENT ON FUNCTION auto_create_documentation_on_status_change() IS 'Creates documentation certificate with verification token when reservation becomes documented';
